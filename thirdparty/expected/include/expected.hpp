...
// in some cases. This is a check to workaround the common failing case.
        ...
        namespace swap_adl_tests {
            ...
            // helper functions to test if an unqualified swap is possible, and
            // if it becomes std::swap
        ...
        struct is_nothrow_swappable
            : std::integral_constant<
                  bool, is_swappable<T, U>::value &&
                            ((decltype(detail::swap_adl_tests::uses_std<T, U>(
                                  0))::value &&
                              detail::swap_adl_tests::is_std_swap_noexcept<
                                  T>::value) ||
                             (!decltype(detail::swap_adl_tests::uses_std<T, U>(
                                  0))::value &&
        ...
        template <class T, class U>
        using is_void_or =
            conditional_t<std::is_void<T>::value, std::true_type, U>;
    ...
    namespace detail {
        ...
        // Implements the storage of the values, and ensures that the destructor
        // is trivial if it can be.
        //
        ...
        struct expected_storage_base {
            ...
            template <class... Args, detail::enable_if_t<std::is_constructible<
        ...
        // This base class provides some handy member functions which can be
        // used in further derived classes
        template <class T, class E>
        struct expected_operations_base : expected_storage_base<T, E> {
            using expected_storage_base<T, E>::expected_storage_base;

            template <class... Args> void construct(Args &&...args) noexcept {
                new (std::addressof(this->m_val))
            ...
            // guarantee. The problematic case is where rhs has a value, but
            // *this does not.
            //
            // This overload handles the case where we can just copy-construct
            // `T` directly into place without throwing.
            template <class U = T,
                      detail::enable_if_t<std::is_nothrow_copy_constructible<
                          U>::value> * = nullptr>
            ...
            // This overload handles the case where we can attempt to create a
            // copy of `T`, then no-throw move it into place if the copy was
            // successful.
            template <
                class U = T,
                detail::enable_if_t<
            ...
            // This overload is the worst-case, where we have to move-construct
            // the unexpected value into temporary storage, then try to copy the
            // T into place. If the construction succeeds, then everything is
            // fine, but if it throws, then we move the old unexpected value
            // back into place before rethrowing the exception.
            template <
                class U = T,
                detail::enable_if_t<
                    !std::is_nothrow_copy_constructible<U>::value &&
            ...
            template <class U = T,
                      detail::enable_if_t<std::is_nothrow_move_constructible<
                          U>::value> * = nullptr>
        ...
        struct expected_operations_base<void, E>
            ...
            // This function doesn't use its argument, but needs it so that code
            // in levels above this can work independently of whether T is void
        ...
        // This specialization is for when T and E are trivially copy
        // constructible
        template <class T, class E,
                  bool = is_void_or<
                      T, TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>::value
...
// move constructible
        ...
        struct expected_copy_assign_base<T, E, false>
            ...
            expected_copy_assign_base(const expected_copy_assign_base &rhs) =
                default;

            expected_copy_assign_base(expected_copy_assign_base &&rhs) =
                default;
            expected_copy_assign_base &
            operator=(const expected_copy_assign_base &rhs) {
                this->assign(rhs);
                return *this;
            }
            expected_copy_assign_base &
...
// move assignable
        ...
        struct expected_move_assign_base<T, E, false>
            ...
            expected_move_assign_base(const expected_move_assign_base &rhs) =
                default;

            expected_move_assign_base(expected_move_assign_base &&rhs) =
                default;
        ...
        struct expected_delete_ctor_base {
            ...
            expected_delete_ctor_base(const expected_delete_ctor_base &) =
                default;
            expected_delete_ctor_base(expected_delete_ctor_base &&) noexcept =
                default;
        ...
        // which follows, while still conditionally deleting the default
        // constructor.
        struct default_constructor_tag {
            explicit constexpr default_constructor_tag() = default;
        };

        // expected_default_ctor_base will ensure that expected has a deleted
        // default consturctor if T is not default constructible. This
        // specialization is for when T is default constructible
        template <class T, class E,
    ...
    /// `E`. The contained object may not be initialized after the expected
    /// object has been initialized, and may not be destroyed before the
    /// expected object has been destroyed. The initialization state of the
    /// contained object is tracked by the expected object.
    template <class T, class E>
    ...
    !defined(TL_EXPECTED_GCC54) && !defined(TL_EXPECTED_GCC55)
        ...
        constexpr expected(in_place_t, std::initializer_list<U> il,
                           Args &&...args)
        ...
        template <class G = E,
                  detail::enable_if_t<std::is_constructible<E, G &&>::value> * =
                      nullptr,
                  detail::enable_if_t<!std::is_convertible<G &&, E>::value> * =
                      nullptr>
        ...
        template <class U, class G,
                  detail::enable_if_t<!(std::is_convertible<U &&, T>::value &&
                      ...
                      * = nullptr,
                  detail::expected_enable_from_other<T, E, U, G, U &&, G &&> * =
                      nullptr>
    ...
    private:
        ...
        void swap_where_both_have_value(expected & /*rhs*/,
                                        t_is_void) noexcept {
